...
export default function AliasesPage() {
	...
	if (loading) return <div className="p-8 text-sm text-[var(--ink-muted)]">Loading…</div>;

	return (
		<div className="space-y-6 max-w-3xl">
			<h1 className="text-2xl font-semibold">Aliases</h1>
			<p className="text-sm text-[var(--ink-muted)]">Create email aliases that forward to mailboxes or external addresses.</p>
			...
			<Card>
				...
				<CardContent>
					{aliases.length === 0 ? (
						<p className="text-sm text-[var(--ink-faint)]">No aliases yet.</p>
					) : (
							...
							{aliases.map((a) => (
								<li key={a.id} className="flex items-center justify-between py-3">
									<div className="flex items-center gap-2 text-sm">
										...
										<ArrowRight className="h-3 w-3 text-[var(--ink-faint)]" />
