...
const SOURCE_BADGE: Record<ContactSource, { label: string; className: string }> = {
	manual: { label: "Manual", className: "bg-[var(--accent-muted)] text-[var(--accent)]" },
	inbound: { label: "Inbound", className: "bg-[var(--success)]/15 text-[var(--success)]" },
	...
	return (
		<div className="space-y-6 p-6">
			<div className="flex items-center justify-between">
				<div>
					<h2 className="text-xl font-semibold text-[var(--ink)]">Contacts</h2>
					<p className="text-sm text-[var(--ink-muted)]">People you have emailed or received mail from.</p>
				</div>
			...
			{showForm && (
				...
				>
					<div className="flex items-center justify-between">
						<h3 className="text-sm font-medium text-[var(--ink)]">New contact</h3>
						<button
							type="button"
							onClick={() => { setShowForm(false); setFormError(null); }}
							className="text-[var(--ink-faint)] hover:text-[var(--ink-muted)]"
						>
							<X className="h-4 w-4" />
						</button>
					</div>
			...
			<div>
				<input
					...
					className="h-9 w-full max-w-sm rounded-md border border-[var(--border)] bg-[var(--surface-subtle)] px-3 text-sm text-[var(--ink)] placeholder:text-[var(--ink-faint)] focus:outline-none focus:ring-2 focus:ring-[var(--border-strong)]"
				/>
			</div>
			...
			{isLoading ? (
				...
				<div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-[var(--border)] py-12 text-center">
					<Users className="mb-3 h-8 w-8 text-[var(--ink-faint)]" />
				...
				<div className="divide-y divide-[var(--border)] rounded-lg border border-[var(--border)] bg-[var(--surface-raised)]">
					{filtered.map((contact) => {
						...
						return (
							...
							>
								<div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-[var(--surface-subtle)] text-sm font-semibold text-[var(--ink)]">
									{getInitial(contact)}
								</div>
								<div className="flex-1 min-w-0">
									{contact.displayName && (
										<p className="text-sm font-medium text-[var(--ink)] truncate">{contact.displayName}</p>
									)}
									<p className="text-sm text-[var(--ink-muted)] truncate">{contact.email}</p>
								</div>
								<span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ${badge.className}`}>
									{badge.label}
								</span>
								<span className="shrink-0 text-xs text-[var(--ink-faint)] tabular-nums">
